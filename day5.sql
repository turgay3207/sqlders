...
1)isme gore toplam maaslari bulunuz?

select isim ,
sum(maas) as toplam_maas from personel group by isim;
...
/*
	...
	Where kullanimi ile ayni mantik calisir
*/
...
select sehir,
count(isim) as toplam_personel_sayisi from personel group by sehir having count(isim)>1;
...
select isim as isci_ismi_veya_sehir_ismi, maas from personel where maas >4000 
UNION select sehir as isci_veya_sehir_ismi, maas from personel where maas>5000;

bu sorgulari birlestiriyor join tablolari birlestirir...
...
Union select sehir,maas from personel where sehir='Istanbul';
...
select isim,maas from personel where maas<5000
-------------------------------SQL INTERSECT--------------------
/*SQL INTERSECT(KESISIM) operatörü, 2 veya daha fazla SELECT ifadesinin sonuçlarını döndürmek için kullanılır.
...
--1)Maasi 4800’den az olanlar veya 5000’den cok olanlarinid’lerini
--listeleyin  2)Personel_bilgi tablosundan 2 veya 3 cocugu olanlarin id leriniyazdirin
...
SELECT isim
FROM personel WHERE sirket='Honda'
INTERSECT
